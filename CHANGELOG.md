...
The format is based on [Conventional Commits][344], and this project adheres to
[Semantic Versioning][345].

## [7.8.4][346] (2021-02-20)
...
- **deps:** bump object-path from 0.11.3 to 0.11.5 ([462e27c][348])
- **deps:** bump object-path from 0.11.3 to 0.11.5 ([e9e3ca1][349])
...
- **build-test-deploy.yml:** better error messages ([6d5573e][3])
- **deps:** bump object-path from 0.11.3 to 0.11.5 ([c493a8c][4])

## [7.8.2][5] (2021-02-20)

### Build System

- **deps:** bump object-path from 0.11.4 to 0.11.5 ([aaf7665][6])

## [7.8.1][7] (2021-02-20)

### Bug Fixes

- **build-test-deploy.yml:** store and use proper result ([09d35d7][8])

### Build System

- **deps:** bump object-path from 0.11.4 to 0.11.5 ([658f591][9])
- **package.json:** bump jest timeout to 1 minute ([cb59af2][10])
...
- **build-test-deploy.yml:** whitelist is again case-insensitive ([c034e86][14])

### Build System

- **build-test-deploy.yml:** also run on reopened PRs ([ece78e3][15])
- **build-test-deploy.yml:** codecov failures no longer fail the whole pipeline
  ([a111d7f][16])
- **build-test-deploy.yml:** restructure env w/ comments ([b980cbe][17])
- **build-test-deploy.yml:** update repo owner whitelist step ([b4f89cf][18])

# [7.7.0][19] (2021-02-19)

### Bug Fixes

- Account for node 12 ([4cdb325][20])
...
- **package.json:** external integration test runs w/ proper prefix
  ([5056c05][22])
- **package.json:** update deps ([4ef9c1b][23])

### Features

- **external-scripts:** reorganize externals; rename external to dummy
  ([efc916e][24])
- **test:** externalize jest helper proto-packages ([2dbfb8e][25])

## [7.6.5][26] (2021-02-15)

### Build System

- **package.json:** add private:false ([090cdfe][27])

## [7.6.4][28] (2021-02-14)

### Bug Fixes

- **webpack.config.js:** call dotenv properly ([22a8424][29])

## [7.6.3][30] (2021-02-14)

### Bug Fixes

- Incorrect package name ([bbbd58d][31])

### Build System

- **package.json:** update deps ([599b6c2][32])
...
- **package.json:** account for new ./dist organization ([99a01ea][35])

### Build System

- **test:** complete testing suite ([35da4c3][36])
- **webpack.config.js:** add externals ([7a8f54b][37])
- Move debug to deps, update dependencies ([6587184][38])
- **.eslintrc.js:** false env browser/webextensions ([71809e9][39])
- **integration-client.test.ts:** removed ([f080ad0][40])
- Normalize across repos ([2020d61][41])
- **webpack.config.js:** normalize webpack configuration across repos
  ([8a3bc81][42])
- **webpack.config.js:** remove ES6 syntax from JS file ([dd2b254][43])
...
- **babel.config.js:** add babel-plugin-transform-default-named-imports
  ([720dc20][45])
- **package.json:** update deps ([26a8371][46])
...
- **post-release-check.yml:** run executables as part of check ([31bdc0c][49])

## [7.5.18][50] (2021-01-29)

### Build System

- **build-test-deploy.yml:** drop support for webpack 4 ([e7f2bbb][51])
- **cleanup.yml:** fix bug for new package repos ([8d16630][52])

## [7.5.17][53] (2021-01-29)

### Build System

- **babel.config.js:** add plugin-rename-imports ([c59e45c][54])
- **build-test-deploy.yml:** remove externals exception ([3addf11][55])
- **test:** drop support for 10.x ([fb18856][56])
- **test:** standardize imports ([4f23d4e][57])

## [7.5.16][58] (2021-01-23)

### Build System

- **package.json:** nicer destructured vals in docs ([3b81944][59])

## [7.5.15][60] (2021-01-23)

### Build System

- Backport/normalize across packages ([85881f7][61])

## [7.5.14][62] (2021-01-22)

### Build System

- Update debug statement syntax ([107b953][63])

## [7.5.13][64] (2021-01-21)
...
## [7.5.12][66] (2021-01-17)

### Build System

- Backport changes from gac ([172fa77][67])

## [7.5.11][68] (2021-01-12)

### Build System

- Rebuild lockfile ([fef7222][69])
- Update babel-plugin-transform-mjs-imports ([16a597d][70])

## [7.5.10][71] (2021-01-12)

### Build System

- Update -browser to -client everywhere ([597d6c5][72])

## [7.5.9][73] (2021-01-10)

### Bug Fixes

- **webpack.config.js:** better process.env support ([25db498][74])

## [7.5.8][75] (2021-01-06)

### Build System

- **webpack.config.js:** process.env shim ([3973279][76])
...
- **package.json:** backported updates ([8a051d1][86])

## [7.5.2][87] (2021-01-05)

### Build System

- **package.json:** backported updates ([2cf6e46][88])

## [7.5.1][89] (2021-01-05)

### Bug Fixes

- **env-expect.js:** use syntax for node 10 ([cb77d20][90])
- **package.json:** return browser test ([6c1c4d4][91])

### Build System

- **.github:** add INCLUDE_EXTERNALS ([c755631][92])
- **build-test-deploy.yml:** remove codecov upload for externals integration
  tests ([3fdd509][93])
- **package.json:** backports from the wild ([eda277e][94])
- **webpack.config.js:** add env-expect ([0f401ff][95])

# [7.5.0][96] (2021-01-05)

### Features

- **package-lock.json:** add unique-filename ([77c033f][97])
...
## [7.4.4][100] (2021-01-04)

### Build System

- **.github/workflows/build-test-deploy.yml:** move webpack install
  responsibility to integration ([5c8fefa][101])

## [7.4.3][102] (2021-01-04)

### Build System

- **.github/workflows/build-test-deploy.yml:** add debug output to npm ci
  ([09304d7][103])
- **.github/workflows/build-test-deploy.yml:** fix debug output ([c247c7a][104])
- **.github/workflows/build-test-deploy.yml:** fix debug output ([6eb7d41][105])
- **.github/workflows/build-test-deploy.yml:** fix debug output (normalize node
  support) ([0528db6][106])

## [7.4.2][107] (2021-01-04)

### Build System

- **.github/workflows/build-test-deploy.yml:** do not run npm install when stdin
  is empty ([2842a31][108])
- **package.json:** revert "test: peer dep install" ([4c174fe][109])
- **package.json:** test: peer dep install ([6c00005][110])

## [7.4.1][111] (2021-01-04)
...
# [7.4.0][114] (2021-01-04)

### Features

- **.github:** add codecov to externals; add flags to codecov ([e47160b][115])

# [7.3.0][116] (2021-01-04)

### Bug Fixes

- **env-expect.js:** simple RegExp names no longer error ([a576d0b][117])

### Build System

- **package.json:** better lint output ([0dc7c4e][118])

### Features

- **.github:** backport env matrix hint ([c01b325][119])
- **readme.md:** add NPM7 warning ([24f0888][120])

## [7.2.3][121] (2021-01-04)

### Build System

- Updates back-ported from package migration ([7869420][122])

## [7.2.2][123] (2021-01-03)

### Bug Fixes

- Add remark frontmatter support ([a8cd378][124])

## [7.2.1][125] (2021-01-03)

### Bug Fixes

- **templates:** fix strange issue template problems ([73edb9a][126])

# [7.2.0][127] (2021-01-03)

### Bug Fixes

- **changelog.md:** conclude tests _breaks champagne glass_ ([864253c][128])
- **workflows:** actually fix should-skip determination algo ([bf8c76b][129])
- **workflows:** fix should-skip determination algo ([f7d1256][130])

# [7.1.0][131] (2021-01-03)

### Bug Fixes

- **.github:** add set +e to should-skip ([27c9750][132])
- **readme.md:** complete skip test ([67264ed][133])
- **workflows:** replace should-skip determination algo ([cd6236a][134])

# [7.0.0][135] (2021-01-03)

### Features

- **changelog.md:** add skip cd/cd skip pipeline commands ([5765c02][136])

### BREAKING CHANGES

- **changelog.md:** **skip github/github skip options were removed**

## [6.7.3][137] (2021-01-03)

### Bug Fixes

- **.changelogrc.js:** show reverts in changelog ([ec26302][138])

### Reverts

- _"chore(changelog.md): update pipeline usage bullets"_ ([6e9fb73][139])

## [6.7.2][140] (2021-01-03)

### Reverts

- _"chore(readme.md): debug chore"_ ([38bfb77][141])
- _"debug(readme.md): debug debug"_ ([aff005d][142])

## [6.7.1][143] (2021-01-02)

### Bug Fixes

- **.changelogrc.js:** exclude pre-releases ([c24773e][144])
- **.changelogrc.js:** fix bad type bug ([ff94fe1][145])
- **.changelogrc.js:** several writer glitches fixed ([717ffdb][146])

### Build System

- **package.json:** better build-changelog ([f2e971e][147])

# [6.7.0][148] (2021-01-02)

### Features

- **.changelogrc.js:** reintroduced selective capitalization ([1fdcb9c][149])

# [6.6.0][150] (2021-01-02)

### Bug Fixes

- **.changelogrc.js:** correct improper array access ([0cb2645][151])

### Build System

- **.changelogrc.js:** fix revision filtering during generation ([ac06b33][152])
- **.changelogrc.js:** fix spread bug ([c6a3cd6][153])

### Features

- **.changelogrc.js:** centralize release rules ([69ec71f][154])
- **release.config.js:** use imported releaseRules ([2fe163c][155])

## [6.5.1][156] (2021-01-02)

### Bug Fixes

- **release.config.js:** add title to releaseRules objects ([99c2868][157])

### Build System

- **.changelogrc.js:** add debug output to .changelogrc.js ([d47c48b][158])

# [6.5.0][159] (2021-01-02)

### Features

- **.changelogrc.js:** add debug to all javascript ([54488d9][160])

## [6.4.3][161] (2021-01-02)
...
- **.github:** fix debug scope name (leave / alone) ([de2c7b1][163])
- **package.json:** remove deploy test script ([4fb641c][164])

## [6.4.2][165] (2021-01-02)

### Build System

- **.github:** add DEBUG support to metadata gathering steps ([6b2e238][166])

## [6.4.1][167] (2021-01-02)

### Build System

- **.github/workflows/build-test-deploy.yml:** add DEBUG support
  ([de7ef33][168])
- **release.config.js:** fix bracket scoping issue ([c8ef15e][169])
- **release.config.js:** fix deploy to run at success step; add debug output
  ([b9e8cc5][170])

# [6.4.0][171] (2021-01-02)

### Features

- **package.json:** vercel deploy test ([ed73fc3][172])

## [6.3.8][173] (2021-01-02)
...
## [6.3.5][179] (2021-01-01)

### Build System

- **.github:** fix hardcoded package name ([a10bac3][180])

## [6.3.4][181] (2021-01-01)

### Build System

- Move npm lint step before lint-staged in husky hook ([e8bac90][182])
- Update remark linting to exit 1 on error, quiet on success ([077abce][183])
- Update sort-package-json to contrib-env-expect ([6885a34][184])
- **pre-commit:** decentralize remark; more resilient build system
  ([ba0ef58][185])

## [6.3.3][186] (2021-01-01)
...
- **pre-commit:** move eslint (and tsc) CL linting out of lint-staged and
  directly into husky ([e38af90][188])

## [6.3.2][189] (2021-01-01)

### Build System

- **.github:** fix pre/post formatting steps ([f757db8][190])

## [6.3.1][191] (2021-01-01)

### Bug Fixes

- **test:** improve test coverage ([7f4a70a][192])

# [6.3.0][193] (2021-01-01)

### Build System

- **package.json:** simplify lint ([dda943e][194])

### Features

- **src:** add div function ([636c353][195])

# [6.2.0][196] (2021-01-01)

### Build System

- **.github/workflows/build-test-deploy.yml:** add remark and formatting steps
  ([e16cd8b][197])
- **.husky:** remove verbose flag ([538c374][198])
- **package.json:** fix lint-staged remark and prettier ([6fbd1df][199])
- **package.json:** remove debug echo ([33a510d][200])
- **release.config.js:** fix to use gfm ([db4c284][201])

### Features

- **package.json:** add remark-gfm; test lint-staged ([bcbf3f5][202])

## [6.1.7][203] (2020-12-31)

### Bug Fixes

- **README.md:** simulated external-PR fix ([8a7fc06][204])

### Reverts

- _"chore(readme.md): format husky test"_ ([9675eeb][205])

## [6.1.6][206] (2020-12-30)
...
## [6.1.4][210] (2020-12-30)

### Reverts

- _"build(build-test-deploy.yml): debug should warn"_ ([44f5cd4][211])

## [6.1.3][212] (2020-12-30)

### Build System

- **build-test-deploy.yml:** debug should warn ([55e1235][213])
- **build-test-deploy.yml:** fix syntax error ([5448960][214])

## [6.1.2][215] (2020-12-30)

### Build System

- **.github:** better snyk ignore ([5921653][216])
- **package.json:** remove high audit CI failure ([b2ea6f1][217])
- **package.json:** test high audit CI failure ([03825f0][218])

## [6.1.1][219] (2020-12-30)

### Bug Fixes

- **readme.md:** dummy fix ([bda2d5e][220])

# [6.1.0][221] (2020-12-30)

### Bug Fixes

- **readme.md:** dummy fix ([b9d92fd][222])

### Features

- **readme.md:** dummy feat ([f90dde3][223])

## [6.0.5][224] (2020-12-30)

### Bug Fixes

- **readme.md:** add codecov badge ([4b6c84c][225])

## [6.0.4][226] (2020-12-30)

### Build System

- **.github:** more resilient cleanup routine ([85b8e12][227])

## [6.0.3][228] (2020-12-30)

### Build System

- **.github:** better cleanup caching ([02746df][229])
- **.github:** fix cleanup ([7520e0f][230])

## [6.0.2][231] (2020-12-30)

### Build System

- **cleanup.yml:** add cleanup workflow ([747a674][232])
- **release.config.js:** remove console.info ([b374296][233])

## [6.0.1][234] (2020-12-30)

### Build System

- **.github:** ensure deploy stage is using latest node and npm ([d40192b][235])

# [6.0.0][236] (2020-12-30)

### Features

- **readme.md:** fake feature ([95a13d2][237])

### BREAKING CHANGES

- **readme.md:** **fake breaking change**

## [5.0.3][238] (2020-12-29)
...
- _"chore(.gitignore): ignore coverage"_ ([4aab0bd][240])

## [5.0.2][241] (2020-12-29)
...
## [5.0.1][243] (2020-12-29)

### Build System

- **.github:** add codecov-action to test-unit job ([2ef6297][244])
- **package.json:** add codecov package ([d73cac9][245])
- **package.json:** remove codecov package ([da477b7][246])

# [5.0.0][247] (2020-12-29)

### Chore

- Fix capitalization of non-release commit types ([7391321][248])
...
- Update comment language ([5a2aef4][251])
...
## [3.0.1][252] (2020-12-28)

### Bug Fixes

- **.github:** update ci committer name and email to xunn-bot ([b151491][253])
- **package.json:** update typedoc to latest beta and fix npmbd ([783ba76][254])

### Build System

- **package.json:** update deps ([a876517][255])

# [3.0.0][256] (2020-12-07)

### Bug Fixes

- **.github/workflows:** fix syntax: "no-ci/\*\*" ([b060d3e][257])
- **config:** rename changelog.config.js to .changelogrc.js ([d5dfc50][258])
- **package.json:** use @xunnamius/semantic-release ([0447d59][259])

### Build System

- **.github/workflows:** build system now ignores branches with names that begin
  with "no-ci/" ([3a4bdff][260])
- **.github/workflows:** workflow no longer triggered on tag push,
  dependabot/snyk push ([1096e6f][261])
- **package-lock.json:** fix syntax error ([4fd2952][262])
...
- Added mult function ([e537358][264])

### Reverts

- _"chore(.github/workflows): exclude ./.git from build caching"_
  ([b75d8ac][265])
- _"debug(.github/workflows): fake change"_ ([d59bb1a][266])
- _"debug(.github/workflows): test force push protection"_ ([4cc6ee6][267])
...
## [2.3.4][268] (2020-12-03)

### Build System

- **commitlint:** add debug type to type-enum ([10c71bf][269])
- **config:** fix changelog.config.js bug when subject is undefined
  ([57e0c16][270])
- **package.json:** downgrade puppeteer and upgrade other deps ([e5d6d42][271])
- Added .env.example and env-expect ([efb29c5][272])

### Reverts

- revert "debug: switch actions ci to install" ([f9a3e23][273])

## [2.3.3][274] (2020-11-30)
...
- **release:** fix changelogTitle skip title ([1dcdcc1][276])

## [2.3.2][277] (2020-11-30)

### Build System

- **release:** fix changelogTitle spread bug ([598a396][278])

## [2.3.1][279] (2020-11-30)

### Build System

- Install @semantic-release/changelog ([022603b][280])
- **release:** fix changelogTitle bug ([0d181ea][281])
- **release:** fix SHOULD_UPDATE_CHANGELOG conditional ([ec632ad][282])

# [2.3.0][283] (2020-11-30)

### Features

- **tooling:** spellcheck now ignores camelCase et al ([d921382][284])

### Performance Improvements

- Better faster spellchecking ([c44c310][285])

## [2.2.2][286] (2020-11-29)
...
## [2.2.1][288] (2020-11-29)

### Build System

- **changelog:** changelog now includes relevant reverts ([d4ccacb][289])

# [2.2.0][290] (2020-11-29)
...
- **config:** changelog config honors skip commands, leaves reverts alone
  ([823f8b5][292])
- **config:** fix broken changelog.config.js structure ([bd10f44][293])
- **package.json:** downgrade typedoc and assoc plugins ([46db2ee][294])
- **package.json:** install lint-staged dep and script ([f3800e4][295])
- **package.json:** install spellcheck dependencies ([9a7adb4][296])
- **spellcheck:** change "contains" to "includes" ([af76dbe][297])
- **spellcheck:** fix bug where spellcheck reads from wrong message source
  ([69a25ba][298])
- **spellcheck:** spellcheck now leverages assorted knowledge ([14b86ae][299])

### Build System

- Install @semantic-release/exec ([cc94940][300])
- **config:** fix changelog.config.js operating on individual releases
  ([66adf2f][301])
- **config:** fix husky pre-commit hook ([f0d0032][302])
- **config:** generate CHANGELOG.md beautifully ([2fa4aa9][303])
- **config:** retire @semantic-release/changelog ([787a281][304])

### Features

- Add build-changelog to npm scripts ([8393a87][305])

## [2.1.4][306] (2020-11-28)

### Bug Fixes

- **actions:** ensure metadata is gathered properly ([31ece1c][307])
- **release:** remove extra newlines during changelogs generation
  ([72acb24][308])

## [2.1.3][309] (2020-11-28)

### Bug Fixes

- **release:** update how changelogs are generated ([8ce1760][310])

## [2.1.2][311] (2020-11-28)

### Bug Fixes

- **actions:** remove erroneous build-dist from lint job ([2cdd373][312])
- **actions:** remove erroneous build-dist from test-unit job ([fcb2232][313])
- **config:** fix babel.config.js with NODE_LTS ([cdf4610][314])

## [2.1.1][315] (2020-11-26)

### Bug Fixes

- **actions:** update gather-metadata with more accurate grep ([961ecef][316])

# [2.1.0][317] (2020-11-25)

### Bug Fixes

- **actions:** gather-metadata steps gather and skip properly ([97682f4][318])
- **actions:** if statements now use | instead of > ([24e3739][319])
- **actions:** move comments out of multiline statements ([874f8d6][320])
...
- **dependabot:** ensure dependabot formats GitHub Actions PRs correctly
  ([bcdbd0e][324])
- Add build commits to release rule ([3124ab0][325])

# [2.0.0][326] (2020-11-24)

### Bug Fixes

- **actions:** add install-dependencies to semantic-release needs
  ([d8cc836][327])
- **actions:** deps caching done properly ([bbe0416][328])
- **actions:** ensure repo is always checked out first ([65e5d46][329])
- **actions:** removed signed pushes ([f621f76][330])
- **package.json:** do not guess error codes ([f5428c8][331])
- **package.json:** ensure machine-friendly run-script syntax ([7af5e45][332])
...
- Added diff to take the difference ([5e40deb][335])

### Performance Improvements

- **actions:** better caching ([6da0a65][336])
- **actions:** made test-integration-chrome independent ([d489cfa][337])
- **actions:** only cache node_modules ([57775c9][338])

### Reverts

- _"debug: added debug step"_ ([1741cde][339])
- _"debug: debug cache miss"_ ([42af347][340])
...
- Version 1.0.15 ([57faa2c][341])

<!---->

- Version 1.0.7 ([5ced626][342])

- Version 1.0.0 ([df91d82][343])

[1]: https://github.com/Xunnamius/projector-lens-lib-cjs/compare/v7.8.2...v7.8.3
[2]:
...
[5]: https://github.com/Xunnamius/projector-lens-lib-cjs/compare/v7.8.1...v7.8.2
[6]:
  https://github.com/Xunnamius/projector-lens-lib-cjs/commit/aaf76651b7632f7f9ac6c957c0d8e74ddd0099ac
[7]: https://github.com/Xunnamius/projector-lens-lib-cjs/compare/v7.8.0...v7.8.1
[8]:
...
[344]: https://conventionalcommits.org
[345]: https://semver.org
[346]:
...
[348]:
  https://github.com/Xunnamius/projector-lens-lib-cjs/commit/462e27c3a055743737fb0ed4a767fd67af5ecb73
[349]:
